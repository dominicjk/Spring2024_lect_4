...
// returns a vector with true for numbers less than the second parameters and fals for those greater than or equal to
...
// adds n to each element of the vector
std::vector<double> AddN(std::vector<double> nums, double n) {
        for (int num : nums) {
        num += n;
    }
    return nums;
...
std::vector<int> SubtractN(std::vector<int> nums, int n) {
    for (int num : nums) {
        num -= n;
    }
    return nums;
